...
fn generateStageClear(allocator: std.mem.Allocator, config: JingleConfig) ![]f32 {
    ...
    // Ascending arpeggio: 1, 3, 5, then root+octave held
    const notes = [_]u8{
        @as(u8, root) + (base_oct + 1) * 12, // C5
        @as(u8, root) + 4 + (base_oct + 1) * 12, // E5
        @as(u8, root) + 7 + (base_oct + 1) * 12, // G5
        @as(u8, root) + (base_oct + 2) * 12, // C6
    };
    ...
    // Add chord for fanfare
    const chord_notes = [_]u8{
        @as(u8, root) + (base_oct + 1) * 12,
        @as(u8, root) + 4 + (base_oct + 1) * 12,
        @as(u8, root) + 7 + (base_oct + 1) * 12,
    };
    const chord_start: usize = @intFromFloat(750.0 * speed * config.sample_rate / 1000.0);
    const chord_dur: usize = @intFromFloat(1000.0 * speed * config.sample_rate / 1000.0);
    ...
    for (chord_notes) |note| {
        ...
        for (0..@min(chord_dur, buf.len -| chord_start)) |i| {
            ...
            if (idx < buf.len) {
                buf[idx] += oscillator.sample(.square, phase) * env.process() * 0.15;
            }
...
fn generateGameOver(allocator: std.mem.Allocator, config: JingleConfig) ![]f32 {
    ...
    // Descending: G4, Eb4, C4, low sustain
    const notes = [_]u8{
        @as(u8, root) + 7 + (base_oct + 1) * 12, // G4
        @as(u8, root) + 3 + (base_oct + 1) * 12, // Eb4
        @as(u8, root) + (base_oct + 1) * 12, // C4
        @as(u8, root) + base_oct * 12, // C3 (low)
    };
    const durations = [_]f32{
        300.0 * speed,
        300.0 * speed,
...
fn generateLevelUp(allocator: std.mem.Allocator, config: JingleConfig) ![]f32 {
    ...
    // Quick ascending: C5, D5, E5, G5, C6
    const notes = [_]u8{
        @as(u8, root) + (base_oct + 1) * 12,
...
fn generateBossClear(allocator: std.mem.Allocator, config: JingleConfig) ![]f32 {
    ...
    // Dramatic resolution: ascending motion then big chord
    const notes = [_]u8{
        @as(u8, root) + 7 + base_oct * 12, // G3
        @as(u8, root) + (base_oct + 1) * 12, // C4
        @as(u8, root) + 4 + (base_oct + 1) * 12, // E4
        @as(u8, root) + 7 + (base_oct + 1) * 12, // G4
        @as(u8, root) + (base_oct + 2) * 12, // C5 held
    };
    const durations = [_]f32{
        300.0 * speed,
        300.0 * speed,
    ...
    // Add power chord at the end
    const chord_start: usize = @intFromFloat(1300.0 * speed * config.sample_rate / 1000.0);
    const chord_dur: usize = @intFromFloat(1500.0 * speed * config.sample_rate / 1000.0);
    const chord_notes = [_]u8{
        @as(u8, root) + (base_oct + 1) * 12,
        @as(u8, root) + 4 + (base_oct + 1) * 12,
        @as(u8, root) + 7 + (base_oct + 1) * 12,
    ...
    for (chord_notes) |note| {
        ...
        for (0..@min(chord_dur, buf.len -| chord_start)) |i| {
            if (i == chord_dur * 6 / 10) env.noteOff();
            const idx = chord_start + i;
            if (idx < buf.len) {
                buf[idx] += oscillator.sample(.square, phase) * env.process() * 0.12;
            }
            phase += freq / config.sample_rate;
            phase -= @floor(phase);
        }
